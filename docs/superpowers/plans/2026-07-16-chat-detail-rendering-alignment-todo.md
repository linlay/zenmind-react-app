...
| [x] | BASE-05 | awaiting question / approval / form / plan | `ChatAwaitingDock.tsx`、`AwaitingFormViewport.native.tsx` | 已支持提交、回答摘要、密码遮罩、form viewport 和选项 HTML 预览。 |
...
| [x] | TODO-13 | P2 | 已接入但有差距 | Markdown resource / workspace 文件链接 | 当前 Markdown 链接是普通链接，没有 Web 的资源下载与 workspace 文件预览路由。 | `MarkdownContent.tsx`、`ContentBlock.tsx` 的 workspace link handler | 识别约定 resource/workspace link；经认证 API 获取；支持文件名、行号定位、预览和下载；普通 http(s) 链接仍走外部打开；非法路径或缺 agent scope 时安全失败。 | `ConversationMarkdownRenderer.tsx` 仅暴露回调，chat feature 负责路由和 service；不得把认证逻辑放进 shared。 |
| [x] | TODO-14 | P2 | 已接入但有差距 | 用户附件预览 / 下载 | 当前 `ChatAttachmentStrip` 主要展示附件状态和摘要，详情消息里的打开、预览、下载能力弱于 Web。 | `AttachmentCard.tsx`、`attachmentPreview.ts` | 图片可查看大图；文本/常见文档进入预览；其他格式可下载或外部打开；上传中、失败、过期 URL、权限失败均有反馈；多个附件布局可用。 | `ChatAttachmentStrip.tsx`、消息附件 row、feature preview 层和资源 service。 |
| [x] | TODO-15 | P2 | 已接入但有差距 | tool 结果深层结构化展示 | 已接 tool/tool-group，但 JSON renderer 偏浅，复杂嵌套参数/结果阅读性和代码结果识别弱于 Web。 | `ToolPill.tsx` 及 tool result renderer | 对象/数组可递归展开，设最大深度和节点数；长文本可折叠/复制；代码/补丁/错误结果用对应样式；超大 payload 不阻塞列表；密码/token 等敏感字段继续遮罩。 | `runtimePayloadDescriptor.ts`、`runtimePayloadRenderers.tsx`；增加深层、循环防护、超大数据测试。 |
| [x] | TODO-16 | P2 | 已接入但有差距 | awaiting 选项 `previewHtml` | timeline 已解析并参与 signature，但 question/plan 选项 UI 没有渲染 HTML 预览。 | `AwaitingAnswerBlock.tsx` 的 option preview | 带 `previewHtml` 的选项显示预览入口或受限内嵌预览；切换选项时内容同步；提交值不受预览影响；无预览保持原布局；恶意 HTML 安全降级。 | `ChatAwaitingDock.tsx` / question state；可复用 TODO-12 的安全预览层。 |
...
| TODO-08 | 2026-07-18 | `context.compact.start/complete/done/failed/error` typed 归一、requestId/compactId 稳定合并、轻量 ContextCompact row、压缩前后 token/节省量/失败原因、旧 record snapshot 迁移；complete 把同一份合并 usageSummary 同步给 Header 并随 context 节点持久化 | compact 专测 6/6、聚焦测试 118/118、typecheck、lint（0 error，1 条既有 warning）、全量 test 428/428、三平台 export build、`xgraph status` 通过 | context 不再经过通用 record renderer，原始 payload/JSON 不进入 UI；状态更新只替换自身 display item，未新增依赖或 SQLite schema |
| TODO-09 | 2026-07-18 | request message variant 协议归一、typed message node、rich snapshot 兼容迁移、steer/remember/learn 共用命令图标与 Web 对齐标签；普通 query 与未知 request 保持原路径 | request variant 专测 5/5、聚焦测试 129/129、typecheck、Prettier 新文件检查、lint（0 error，1 条既有 warning）、全量 test 433/433、三平台 export build、`xgraph status` 通过 | 命令请求沿用单一 message 节点和 request display registry，不进入基础 SQLite message 投影；实时与 history replay 共用 reducer，未新增 store、schema 或重复条件分支 |
...
| TODO-13 | 2026-07-18 | resource/workspace href 纯分类与安全校验、当前 agent scope 重建 workspace URL、单一 chat Markdown wrapper/列表根 Provider、认证 image/text/PDF 预览与下载、文件名/来源路径/目标行高亮定位；Artifact 同步复用通用资源层 | 链接/资源聚焦测试 42/42、typecheck、Prettier 新文件检查、lint（0 error，1 条既有 warning）、全量 test 439/439、三平台 export build、`xgraph status` 通过 | shared 只保留 href 回调和普通外链默认行为；解析只在点击时执行，预览状态不进入 row/timeline/SQLite，文本目标行使用三段 Text 而非逐行节点，非法路径与缺 agent scope 不发请求 |
| TODO-14 | 2026-07-18 | 页面级单例认证资源预览控制器、消息/输入区附件点击分流、图片大图与 text/PDF 预览、未知格式下载入口、上传失败/缺资源/过期或权限错误反馈；Artifact 与 Markdown 同步移除各自的行级 Modal | 附件分流与单例架构专测 5/5、关联聚焦测试 16/16、typecheck、lint（0 error，1 条既有 warning）、全量 test 444/444、三平台 export build、`xgraph status` 通过 | 附件预览状态不进入 row/timeline/SQLite；列表图片只在 row 可见时解析认证 source，横向多附件仍使用单一 ScrollView；未新增依赖、schema 或资源请求旁路 |
| TODO-15 | 2026-07-18 | tool 参数/结果惰性结构化投影、对象/数组递归展开、字段级长文本分页与收起、代码/补丁/错误语义样式、复制时统一深层脱敏 | 聚焦测试 19/19、typecheck、lint（0 error，1 条既有 warning）、全量 test 452/452、三平台 export build、`xgraph status` 通过 | 折叠列表不解析/压缩 payload；展开后限制 64 KiB 解析、7 层深度、240 节点、每层 48 子项和 64 KiB 树文本；未新增 store、schema、依赖或 timeline 分支 |
| TODO-16 | 2026-07-18 | awaiting question 选项独立 HTML 预览入口、详情页单例 ConversationPreviewProvider、选择/预览同级交互、按会话隔离 overlay 并共享可见性、惰性有界 source hash | 聚焦测试 31/31、typecheck、lint（0 error，1 条既有 warning）、全量 test 456/456、三平台 export build、`xgraph status` 通过 | 复用 TODO-12 的离线 opaque-origin sandbox runtime；预览不改变选中草稿或 submit params，无 previewHtml 不挂载额外按钮；未新增 store、schema、协议或依赖 |
