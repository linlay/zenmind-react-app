...
describe('markdownAssetUrl', () => {
  ...
  it('builds /api/ap/data url with encoded file and optional download', () => {
  ...
  it('resolves markdown image url: http(s) untouched, others to /api/ap/data', () => {
    expect(resolveMarkdownImageUrl('https://cdn.example.com/a.png', backendUrl))
      .toBe('https://cdn.example.com/a.png');
    expect(resolveMarkdownImageUrl('/data/sample_photo.jpg', backendUrl))
      .toBe('https://api.example.com/api/ap/data?file=%2Fdata%2Fsample_photo.jpg');
    expect(resolveMarkdownImageUrl('sample photo.jpg', backendUrl))
      .toBe('https://api.example.com/api/ap/data?file=sample%20photo.jpg');
    expect(resolveMarkdownImageUrl('/api/ap/data?file=%2Fdata%2Fnested.png', backendUrl))
      .toBe('https://api.example.com/api/ap/data?file=%2Fdata%2Fnested.png');
  });
  ...
  it('returns safe fallback for empty input', () => {
    expect(resolveMarkdownImageUrl('', backendUrl)).toBe('');
